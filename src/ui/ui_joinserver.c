...
void M_AddAddressToList(const char *address) {
  ...
  for (int i = 0; i < m_num_addresses; i++) {
    if (!strcmp(local_server_addresses[i], address))
      return;
  }

  Q_strncpyz(local_server_addresses[m_num_addresses++], address,
             sizeof(local_server_addresses[0]));
...
static void M_QueryMaster(void) {
  ...
  byte packet[] = {0xff, 0xff, 0xff, 0xff, 'g', 'e', 't', 's', 'e',
                   'r',  'v',  'e',  'r',  's', ' ', '3', '4', '\n'};
...
static void CopyServerTitle(char *buf, int size, const serverStatus_t *status) {
  ...
  Com_sprintf(map, sizeof(map), " %-10s %2i/%2i %4i",
              (s && s[0]) ? s : "unknown", status->numPlayers, max_clients,
              status->ping);

  int map_len = strlen(map);
  int max_avail = (titleMax > 0 && titleMax < size) ? titleMax : size;
  int host_limit = max_avail - map_len - 1;
...
void CopyServerTitle(char *buf, int size, const serverStatus_t *status)
{
        ...
        Com_sprintf(map, sizeof(map), " %-10s %2i/%2i", s, status->numPlayers,
clients );
...
void M_AddToServerList(const serverStatus_t *status) {
  ...
  if (!joinMenuInitialized || m_num_servers >= MAX_MENU_SERVERS)
    return;

  for (i = 0; i < m_num_servers; i++) {
    if (!strcmp(status->address, localServers[i].address))
      return;
  }
  ...
  CopyServerTitle(local_server_names[m_num_servers],
                  sizeof(local_server_names[0]), &localServers[m_num_servers]);
  ...
  // Gwarantowany terminator wewnątrz granic tablicy (server_shit ma rozmiar
  // MAX_MENU_SERVERS + 1)
...
static void JoinServer_InfoDraw(void) {
  ...
  if (!server->numPlayers) {
    ...
    for (i = 0, player = server->players; i < server->numPlayers;
         i++, player++) {
      const char *pstr =
          va("%-15s %5i %4i", player->name, player->score, player->ping);
...
static void JoinServer_MenuDraw(menuframework_s *self) {
  ...
  DrawString((viddef.width - (strlen(join_menu_title) * 8)) >> 1, 10,
             join_menu_title);
  JoinServer_InfoDraw();
  Menu_Draw(self);
}
...
static void JoinServer_MenuInit(void) {
  ...
  s_joinserver_sort_box.generic.x =
      x - 10 + strlen(s_joinserver_sort_box.generic.name) * 8;
