...
static void CL_ParsePacketEntities(sizebuf_t *msg, const frame_t *oldframe,
  ...
  if (!oldframe)
    ...
    if (oldindex >= oldframe->num_entities)
      ...
      oldstate = &cl_parse_entities[(oldframe->parse_entities + oldindex) &
                                    PARSE_ENTITIES_MASK];
  ...
  while (1) {
    ...
    while (oldnum <
           newnum) { // one or more entities from the old packet are unchanged
    ...
    if (bits & U_REMOVE) { // the entity present in oldframe is not in the
                           // current frame
      if (cl_shownet->integer == 3)
        Com_Printf("   remove: %i\n", newnum);
      if (oldnum != newnum)
    ...
    if (oldnum > newnum) { // delta from baseline
      ...
      if (newframe->num_entities < MAX_VISIBLE_ENTITIES)
        CL_DeltaEntity(msg, newframe, newnum, &cl_entities[newnum].baseline,
                       bits);
      else
        CL_DeltaEntity_Discard(msg, newnum, &cl_entities[newnum].baseline,
                               bits);
...
// r1: this fakes a protocol 34 packetentites write from the clients state
// instead of the server. used to write demo stream regardless of c/s protocol
// in use.
static void CL_DemoPacketEntities(sizebuf_t *buf,
                                  const frame_t /*@null@*/ *from,
                                  const frame_t *to) {
  ...
  while (newindex < to->num_entities || oldindex < from_num_entities) {
    if (newindex >= to->num_entities)
      ...
      newent = &cl_parse_entities[(to->parse_entities + newindex) &
                                  PARSE_ENTITIES_MASK];
    ...
    if (oldindex >= from_num_entities)
      ...
      oldent = &cl_parse_entities[(from->parse_entities + oldindex) &
                                  PARSE_ENTITIES_MASK];
    ...
    if (newnum == oldnum) { // delta update from old position
      ...
      MSG_WriteDeltaEntity(oldent, newent, buf, false,
                           newent->number <= cl.maxclients);
    ...
    if (newnum < oldnum) { // this is a new entity, send it from the baseline
      MSG_WriteDeltaEntity(&cl_entities[newnum].baseline, newent, buf, true,
                           true);
      newindex++;
      continue;
    }
...
void CL_ParseFrame(sizebuf_t *msg, int extrabits) {
  ...
  if (cl.frame.deltaframe > 0) {
    ...
    } else if (!oldframe->valid) { // should never happen
      Com_Printf("Delta from invalid frame (not supposed to happen!).\n");
    }
    if (oldframe->serverframe !=
        cl.frame.deltaframe) { // The frame that the server did the delta from
      // is too old, so we can't reconstruct it properly.
      Com_DPrintf("Delta frame too old.\n");
    } else if (cl.parse_entities - oldframe->parse_entities >
  ...
  if (cls.serverProtocol > PROTOCOL_VERSION_DEFAULT) {
    ...
    if (cls.demorecording) {
      ...
      MSG_WriteDeltaPlayerstate_Default(oldstate, &cl.frame.playerstate,
                                        &fakeMsg);
  ...
  if (cls.state != ca_active) {
    ...
    // cl.frame.servertime = (cl.frame.serverframe - cl.initial_server_frame) *
    // 100;
  ...
  for (pnum = 0; pnum < frame->num_entities; pnum++) {
    ...
    /*		else if (renderfx & RF_BEAM)
                    {	// interpolate start and end points for beams
                            ent.oldorigin[0] = cent->prev.old_origin[0] +
       cl.lerpfrac * (cent->current.old_origin[0] - cent->prev.old_origin[0]);
                            ent.oldorigin[1] = cent->prev.old_origin[1] +
       cl.lerpfrac * (cent->current.old_origin[1] - cent->prev.old_origin[1]);
                            ent.oldorigin[2] = cent->prev.old_origin[2] +
       cl.lerpfrac * (cent->current.old_origin[2] - cent->prev.old_origin[2]);

                            ent.origin[0] = cent->prev.origin[0] + cl.lerpfrac *
       (cent->current.origin[0] - cent->prev.origin[0]); ent.origin[1] =
      ...
      ent.origin[0] = ent.oldorigin[0] =
          cent->prev.origin[0] +
          cl.lerpfrac * (cent->current.origin[0] - cent->prev.origin[0]);
      ent.origin[1] = ent.oldorigin[1] =
          cent->prev.origin[1] +
          cl.lerpfrac * (cent->current.origin[1] - cent->prev.origin[1]);
      ent.origin[2] = ent.oldorigin[2] =
          cent->prev.origin[2] +
          cl.lerpfrac * (cent->current.origin[2] - cent->prev.origin[2]);
    }

    // create a new entity

    // tweak the color of beams
    if (renderfx & RF_BEAM) { // the four beam colors are encoded in 32 bits of
                              // skinnum (hack)
      ent.alpha = 0.30f;
      ...
      if (Developer_searchpath() == 2) {
        ...
        if (renderfx & RF_SHELL_DOUBLE) {
          ...
          if (renderfx & RF_SHELL_RED)
            renderfx |= RF_SHELL_BLUE;
          // if we have a blue shell (and not a red shell), turn it to cyan by
          // adding green
    ...
    if (s1->modelindex2) {
      ...
      // replaces the previous version which used the high bit on modelindex2 to
      // determine transparency
      if (!Q_stricmp(cl.configstrings[CS_MODELS + (s1->modelindex2)],
...
static void CL_AddViewWeapon(const player_state_t *ps,
  ...
  for (pnum = 0; pnum < cl.frame.num_entities; pnum++) {
    s1 = &cl_parse_entities[(cl.frame.parse_entities + pnum) &
                            PARSE_ENTITIES_MASK];
    if (s1->number != cl.playernum + 1)
      continue;

    if (s1->effects &
        (EF_COLOR_SHELL | EF_QUAD | EF_PENT | EF_DOUBLE | EF_HALF_DAMAGE)) {
...
static void CL_CalcViewValues(void) {
  ...
  if (!demoplayback && cl_predict->integer &&
  ...
  if (demoplayback) {
    if (Key_IsDown(K_SHIFT)) {
      ...
      cl.refdef.viewangles[0] =
          LerpAngle(ops->viewangles[0], ps->viewangles[0], lerp);
      cl.refdef.viewangles[1] =
          LerpAngle(ops->viewangles[1], ps->viewangles[1], lerp);
      cl.refdef.viewangles[2] =
          LerpAngle(ops->viewangles[2], ps->viewangles[2], lerp);
    }
  } else if (ps->pmove.pm_type < PM_DEAD) { // use predicted values
    VectorCopy(cl.predicted_angles, cl.refdef.viewangles);
  } else if (ops->pmove.pm_type < PM_DEAD &&
             cls.serverProtocol >
                 PROTOCOL_VERSION_DEFAULT) { // r1: fix for server no longer
                                             // sending viewangles every frame.
    cl.refdef.viewangles[0] =
        LerpAngle(cl.predicted_angles[0], ps->viewangles[0], lerp);
    cl.refdef.viewangles[1] =
        LerpAngle(cl.predicted_angles[1], ps->viewangles[1], lerp);
    cl.refdef.viewangles[2] =
        LerpAngle(cl.predicted_angles[2], ps->viewangles[2], lerp);
  } else { // just use interpolated values
    cl.refdef.viewangles[0] =
        LerpAngle(ops->viewangles[0], ps->viewangles[0], lerp);
    cl.refdef.viewangles[1] =
  ...
  cl.refdef.viewangles[0] +=
      LerpAngle(ops->kick_angles[0], ps->kick_angles[0], lerp);
  cl.refdef.viewangles[1] +=
      LerpAngle(ops->kick_angles[1], ps->kick_angles[1], lerp);
  cl.refdef.viewangles[2] +=
      LerpAngle(ops->kick_angles[2], ps->kick_angles[2], lerp);
...
void CL_AddEntities(void) {
  ...
  int player_updates =
      cl.settings[SVSET_PLAYERUPDATES] ? cl.settings[SVSET_PLAYERUPDATES] : 0;
  ...
  if (cl.time > cl.serverTime) {
    ...
    cl.playerlerp = 1.0f;
    cl.modelfrac = 1.0f;
  } else if (cl.time < cl.serverTime - (1000 / server_fps)) {
    if (cl_showclamp->integer)
      Com_Printf("low clamp %i\n",
                 cl.serverTime - (1000 / server_fps) - cl.time);
    cl.time = cl.serverTime - (1000 / server_fps);
    cl.lerpfrac = 0;
    cl.playerlerp = 0;
    cl.modelfrac = 0;
  } else {
    cl.modelfrac =
        0.5f - ((int)(cl.serverTime / 100) * 100 - cl.time) * (10.0f / 1000.0f);
    cl.lerpfrac = 1.0f - (cl.serverTime - cl.time) * (server_fps / 1000.0f);
    if (cl.player_update_time)
      cl.playerlerp = 1.0f - ((cl.player_update_time - cl.time) * 0.01f) *
                                 (player_updates + 1);
    else
        ...
        void CL_GetEntitySoundOrigin (int ent, vec3_t org)
{
        ...
        {
                // Calculate origin
                org[0] = cent->current.old_origin[0] + (cent->current.origin[0]
- cent->current.old_origin[0]) * cl.lerpfrac; org[1] =
cent->current.old_origin[1] + (cent->current.origin[1] -
cent->current.old_origin[1]) * cl.lerpfrac; org[2] = cent->current.old_origin[2]
+ (cent->current.origin[2] - cent->current.old_origin[2]) * cl.lerpfrac;
        }
        else
        {
                // Calculate origin
                org[0] = cent->prev.origin[0] + (cent->current.origin[0] -
cent->prev.origin[0]) * cl.lerpfrac; org[1] = cent->prev.origin[1] +
(cent->current.origin[1] - cent->prev.origin[1]) * cl.lerpfrac; org[2] =
cent->prev.origin[2] + (cent->current.origin[2] - cent->prev.origin[2]) *
cl.lerpfrac;
        }
        ...
        if (cent->current.solid == 31)
        {
                vec3_t		midPoint;
                cmodel_t	*cmodel =
cl.model_clip[cent->current.modelindex];
