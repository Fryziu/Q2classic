...
==============
*/
static void SCR_LoadPCX(const char *filename, byte **pic, byte **palette,
...
cblock_t Huff1Decompress(cblock_t in) {
  ...
  count =
      in.data[0] + (in.data[1] << 8) + (in.data[2] << 16) + (in.data[3] << 24);
  input = in.data + 4;
...
qboolean SCR_DrawCinematic(void) {
  ...
  Draw_StretchRaw(0, 0, viddef.width, viddef.height, cin.width, cin.height,
                  cin.pic);
