...
typedef struct {
  ...
  // float	minlight;			// don't add when contributing
  // less
} cdlight_t;
...
#ifdef GL_QUAKE
void Draw_String(int x, int y, const char *s, int color, float alpha,
                 qboolean alt);
...
#define DrawColorString(x, y, s, c, a)                                         \
  Draw_String((x), (y), (s), (c), (a), false)
#define DrawString2(x, y, s, a)                                                \
  DrawCString((x), (y), (s), (a), (cl_textcolors->integer))
#else
...
#define Draw_String(x, y, s, c, a, alt)                                        \
  ((alt) ? DrawAltString((x), (y), (s)) : DrawString((x), (y), (s)))
#define DrawColorString(x, y, s, c, a)                                         \
  (((c) & 2) ? DrawAltString((x), (y), (s)) : DrawString((x), (y), (s)))
#endif
...
void CL_ParticleEffect(const vec3_t org, const vec3_t dir, int color,
                       int count);
void CL_ParticleEffect2(const vec3_t org, const vec3_t dir, int color,
                        int count);

// RAFAEL
void CL_ParticleEffect3(const vec3_t org, const vec3_t dir, int color,
                        int count);
...
// PGM
void CL_BlasterParticles2(const vec3_t org, const vec3_t dir,
                          unsigned int color);
void CL_BlasterTrail2(const vec3_t start, const vec3_t end);
void CL_DebugTrail(const vec3_t start, const vec3_t end);
void CL_SmokeTrail(const vec3_t start, const vec3_t end, int colorStart,
...
void CL_GenericParticleEffect(const vec3_t org, const vec3_t dir, int color,
                              int count, int numcolors, int dirspread,
                              float alphavel);
void CL_BubbleTrail2(const vec3_t start, const vec3_t end, int dist);
void CL_Heatbeam(const vec3_t start, const vec3_t end);
void CL_ParticleSteamEffect(const vec3_t org, const vec3_t dir, int color,
...
void CL_ColorFlash(const vec3_t pos, int ent, int intensity, float r, float g,
                   float b);
...
void CL_ParticleSmokeEffect(const vec3_t org, const vec3_t dir, int color,
                            int count, int magnitude);
...
void CL_ParseDelta(sizebuf_t *msg, const entity_state_t *from,
...
void CL_DiminishingTrail(const vec3_t start, const vec3_t end, centity_t *old,
                         int flags);
