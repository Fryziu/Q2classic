...
static request_t *CL_AddRequest(const netadr_t *adr, requestType_t type) {
  ...
  r->adr = *adr;
  r->type = type;
  r->sendtime = cls.realtime;

  return r;
}

/// Cmd_ForwardToServer //adds the current command line as a clc_stringcmd to
/// the client message.
...
/// CL_SendConnectPacket // We have gotten a challenge from the server, so try
/// and connect.
...
static void CL_SendConnectPacket(int useProtocol) {
  ...
  if (cls.serverProtocol == PROTOCOL_VERSION_R1Q2)
    Netchan_OutOfBandPrint(NS_CLIENT, &adr, "connect %i %i %i \"%s\" %u %u\n",
                           ...
                           Cvar_Userinfo(), net_maxmsglen->integer,
...
static void CL_Connect_f(void) {
  ...
  if (!NET_CompareAdr(&adr, &cls.netchan.remote_address)) {
    Com_DPrintf("Resetting protocol attempt since %s is not ",
                NET_AdrToString(&adr));
...
void CL_DumpServerInfo(const serverStatus_t *status) {
  ...
  if (status->numPlayers) {
    for (i = 0, player = status->players; i < status->numPlayers;
         i++, player++) {
...
static void CL_ParsePrintMessage(sizebuf_t *msg, netadr_t *remoteAdr) {
  ...
  if (!type) {
    if (cls.state > ca_disconnected && cls.state < ca_active &&
        NET_CompareBaseAdr(&net_from, remoteAdr)) {
      Com_Printf("%s", string);
      return;
    }
  ...
  case REQ_PING:
    if (CL_ServerStatusResponse(string, &net_from, &serverStatus)) {
      serverStatus.ping = cls.realtime - r->sendtime;
...
void CL_PingServers_f(void) {
  ...
  Netchan_OutOfBandPrint(NS_CLIENT, &adr,
                         va("info %i", PROTOCOL_VERSION_DEFAULT));
...
static void CL_ParseServersResponse(sizebuf_t *msg) {
  ...
  // Omijamy nagłówek ręcznie, by nie polegać na MSG_ReadString (bezpieczeństwo
  // Carmacka)
  ...
  for (i = 0; (msg->readcount + i + 6) <= msg->cursize; i += 6) {
    ...
    Com_sprintf(adrstr, sizeof(adrstr), "%i.%i.%i.%i:%i", adr.ip[0], adr.ip[1],
                adr.ip[2], adr.ip[3], BigShort(adr.port));
...
static void CL_ConnectionlessPacket(sizebuf_t *msg) {
  ...
  if (!strcmp(c, "client_connect")) {
    ...
    Netchan_Setup(NS_CLIENT, &cls.netchan, &net_from, cls.serverProtocol,
                  cls.quakePort);
    ...
    for (i = 1; i < Cmd_Argc(); i++) {
      ...
      if (!strncmp(p, "ac=", 3)) {
        ...
        p += 9;
        if (strlen(p) > 2) {
          Com_sprintf(cls.downloadReferer, sizeof(cls.downloadReferer),
                      "quake2://%s",
                      NET_AdrToString(&cls.netchan.remote_address));
          CL_SetHTTPServer(p);
    ...
    if (try_to_use_anticheat) {
      ...
      S_StopAllSounds();
      Com_Printf("\1\35\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36"
      ...
      if (try_to_use_anticheat > 1)
        Com_Printf("Anticheat required by server, but no anticheat support "
                   "linked in.\n");
#endif
    }
  ...
  if (!strcmp(c, "challenge")) {
    ...
    // available protocol versions now in challenge, until few months we still
    // default to brute.
...
void CL_ReadPackets(void) {
  ...
  if (cls.state >= ca_connected &&
      cls.realtime - cls.netchan.last_received >
          (unsigned int)(cl_timeout->integer * 1000) &&
      !cls.demoplaying) {
  ...
  if (precache_check == CS_MODELS) { // confirm map
    ...
    if (allow_download_maps->integer) {
      if (strlen(cl.configstrings[CS_MODELS + 1]) >= MAX_QPATH - 1)
        Com_Error(ERR_DROP, "Bad map configstring '%s'",
...
redoSkins:;
        ...
        if (precache_model_skin == -1) {
          ...
          if (LittleLong(*(uint32 *)precache_model) == IDALIASHEADER) {
            ...
            while (precache_model_skin - 1 < LittleLong(pheader->num_skins)) {
              ...
              if (strchr(skinname, '\\')) {
                Com_Printf(
                    "Warning, model %s with incorrectly linked skin: %s\n",
                    cl.configstrings[precache_check], skinname);
              } else if (strlen(skinname) > MAX_SKINNAME - 1) {
                Com_Error(ERR_DROP, "Model %s has too long a skin path: %s",
                          cl.configstrings[precache_check], skinname);
              }
            ...
            while (precache_model_skin - 1 <
                   LittleLong(spriteheader->numframes)) {
              skinname = spriteheader->frames[(precache_model_skin - 1)].name;

              // r1: spam warning for models that are broken
              if (strchr(skinname, '\\')) {
                Com_Printf(
                    "Warning, sprite %s with incorrectly linked skin: %s\n",
                    cl.configstrings[precache_check], skinname);
              } else if (strlen(skinname) > MAX_SKINNAME - 1) {
                Com_Error(ERR_DROP, "Sprite %s has too long a skin path: %s",
                          cl.configstrings[precache_check], skinname);
              }
  ...
  if (precache_check >= CS_PLAYERSKINS &&
    ...
    if (allow_download_players->integer) {
      while (precache_check < CS_PLAYERSKINS + MAX_CLIENTS * PLAYER_MULT) {
        ...
        default:
          while (n < PLAYER_MULT) {
            Com_sprintf(fn, sizeof(fn), "players/%s/%s", model,
                        sexedSounds[n - 5]);
            n++;
            if (!CL_CheckOrDownloadFile(fn)) {
  ...
  if (precache_check == ENV_CNT) {
    ...
    if (map_checksum &&
        map_checksum !=
            (uint32)strtoul(cl.configstrings[CS_MAPCHECKSUM], NULL, 10)) {
      Com_Error(ERR_DROP,
                "Local map version differs from server: 0x%.8x != 0x%.8x",
  ...
  if (precache_check > ENV_CNT && precache_check < TEXTURE_CNT) {
    if (allow_download->integer && allow_download_maps->integer) {
      while (precache_check < TEXTURE_CNT) {
        ...
        if (n & 1)
          Com_sprintf(fn, sizeof(fn), "env/%s%s.pcx", cl.configstrings[CS_SKY],
                      env_suf[n / 2]);
        else
          Com_sprintf(fn, sizeof(fn), "env/%s%s.tga", cl.configstrings[CS_SKY],
                      env_suf[n / 2]);
        if (!CL_CheckOrDownloadFile(fn))
          return; // started a download
      }
...
static void OnChange_Protocol(cvar_t *self, const char *oldValue) {
  ...
  if (self->integer != PROTOCOL_VERSION_DEFAULT &&
    ...
    Com_Printf("Protocol %i is not supported. Current supported protocols: "
               "34(vq2) and %i(r1q2)\n",
               self->integer, PROTOCOL_VERSION_R1Q2);
...
void OnChange_http_max_connections(cvar_t *self, const char *oldValue) {
  ...
  if (self->integer > 2)
    Com_Printf(
        "WARNING: Changing the maximum connections higher than 2 violates the "
        "HTTP specification recommendations. Doing so may result in you being "
        "blocked from the remote system and offers no performance benefits "
        "unless you are on a very high latency link (ie, satellite)\n");
}
#endif
...
char *CL_Mapname(void) { return cls.mapname; }
...
static void CL_Health_m(char *buffer, int bufferSize) {
  ...
  Com_sprintf(buffer, bufferSize, "%i",
              (int)cl.frame.playerstate.stats[STAT_HEALTH]);
}
...
static void CL_Ammo_m(char *buffer, int bufferSize) {
  ...
  Com_sprintf(buffer, bufferSize, "%i",
              (int)cl.frame.playerstate.stats[STAT_AMMO]);
}
...
qboolean CL_IsDisconnected(void) { return (cls.state <= ca_disconnected); }
...
void CL_InitLocal(void) {
  ...
  // Nie potrzebujemy trzymać wskaźników do tych cvarów przez cały czas –
  // odczytamy ich wartości wtedy, gdy będą potrzebne. Flaga CVAR_ARCHIVE
  // sprawi, że ich wartości będą zapisywane w konfiguracji gracza.
  ...
  OnChange_http_max_connections(cl_http_max_connections,
                                cl_http_max_connections->resetString);
#endif
...
void CL_Frame(int msec) {
  static int packet_delta, render_delta, misc_delta = 1000;
  qboolean packet_frame = true;
  qboolean render_frame = true;
  ...
  if (!cl_timedemo->integer) {
    if (cls.state == ca_connected) {
      ...
      if (render_delta < 100)
        render_frame = false; // don't flood packets out while connecting
    } else {
      if (packet_delta < 1000 / cl_maxfps->integer)
        packet_frame = false; // packetrate is too high
      if (render_delta < 1000 / r_maxfps->integer)
        render_frame = false; // framerate is too high
    }

    if (!packet_frame && !render_frame)
      return;
  }

  cls.realtime = curtime;
  cls.frametime = packet_delta * 0.001f;
  // don't extrapolate too far ahead
  if (cls.frametime > 0.2f)
    cls.frametime = 0.2f;

  CL_DemoFrame(packet_delta);
  ...
  if (render_frame) {
    ...
    if (host_speeds->integer) {
      extern unsigned int time_before_ref, time_after_ref;
    ...
    if (misc_delta > 99) { // don't need to do this stuff much.
      ...
      if (cls.spamTime && cls.spamTime < cls.realtime) {
        Cbuf_AddText("say \"" APPLICATION " v" VERSION " " CPUSTRING
                     " " BUILDSTRING "\"\n");
