...
#define VERSION "2026-03-15_clean_compilation"
...
void MSG_WriteDeltaUsercmd(sizebuf_t *sb, const struct usercmd_s *from,
                           const struct usercmd_s *cmd, int protocol);
...
void MSG_WriteDeltaPlayerstate_Default(const player_state_t *from,
...
void MSG_ReadDeltaUsercmd(sizebuf_t *sb, const struct usercmd_s *from,
...
void MSG_ParseDeltaEntity(sizebuf_t *msg, const entity_state_t *from,
                          entity_state_t *to, int number, int bits,
                          int protocol);

void MSG_ParseDeltaPlayerstate_Default(sizebuf_t *msg,
                                       const player_state_t *from,
                                       player_state_t *to, int flags);
void MSG_ParseDeltaPlayerstate_Enhanced(sizebuf_t *msg,
                                        const player_state_t *from,
                                        player_state_t *to, int flags,
                                        int extraflags);

int ZLibCompressChunk(byte *in, int len_in, byte *out, int len_out, int method,
                      int wbits);
int ZLibDecompress(byte *in, int inlen, byte /*@out@*/ *out, int outlen,
                   int wbits);
...
#define UPDATE_BACKUP                                                          \
  16 // copies of entity_state_t to keep buffered must be power of two
...
typedef enum { SVSET_PLAYERUPDATES, SVSET_FPS, SVSET_MAX } serversetting_t;
...
// but remote servers can also send across commands and entire text files can be
// execed.
...
void Cmd_CommandCompletion(const char *partial,
...
void Cvar_CommandCompletion(const char *partial,
                            void (*callback)(const char *name,
                                             const char *value));
...
// #define	MAX_MSGLEN		1400		// max length of a
// message
...
int NET_SendPacket(netsrc_t sock, int length, const void *data,
                   const netadr_t *to);
...
#define NET_IsLocalAddress(adr)                                                \
  ((adr)->type == NA_LOOPBACK || ((adr)->ip[0] == 127 && (adr)->ip[1] == 0 &&  \
                                  (adr)->ip[2] == 0 && (adr)->ip[3] == 1))
...
void Netchan_Setup(netsrc_t sock, netchan_t *chan, netadr_t *adr, int protocol,
                   int qport);

// qboolean Netchan_NeedReliable (netchan_t *chan);
int Netchan_Transmit(netchan_t *chan, int length, const byte *data);
void Netchan_OutOfBand(int net_socket, const netadr_t *adr, int length,
                       const byte *data);
void Netchan_OutOfBandPrint(int net_socket, const netadr_t *adr,
                            const char *format, ...);
...
trace_t CM_TransformedBoxTrace(const vec3_t start, const vec3_t end,
                               ...
                               int headnode, int brushmask, const vec3_t origin,
                               const vec3_t angles);
...
// r1: use variadic macros where possible to avoid overhead of evaluations and
// va
/*#if  (__STDC_VERSION__ >= 199901L || _MSC_VER >= 1400 || __GNUC__ >= 3 ||
(__GNUC__ == 2 && __GNUC_MINOR__ >= 95))
// #define Com_DPrintf(...) do{ if(developer->integer) Com_Printf(__VA_ARGS__);
}while(0) #define Com_DPrintf(...) if (developer->integer) {
Com_Printf(__VA_ARGS__); } #endif*/
...
#define Q_IsColorString(p)                                                     \
  (p && *(p) == Q_COLOR_ESCAPE && *((p) + 1) >= '0' && *((p) + 1) <= '7')
