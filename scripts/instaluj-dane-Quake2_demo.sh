...
# Upewnij się, że skrypt pracuje w głównym katalogu projektu (root)
# Zakładamy, że skrypt znajduje się w <root>/scripts/
...
# Krok 7: Instalacja skompilowanych plików i zależności
echo -e "${YELLOW}Krok 7: Kopiowanie plików binarnych i konfiguracji...${NC}"

# 1. Główne binaria
if [[ ! -f "bin/q2classic" ]] || [[ ! -f "bin/q2game.so" ]]; then
    echo -e "${RED}BŁĄD: Skompilowane pliki nie zostały znalezione w bin/.${NC}"; exit 1;
fi
cp bin/q2classic "$INSTALL_DIR/"
cp bin/q2game.so "$INSTALL_DIR/baseq2/"

# 2. Opcjonalna biblioteka libphonon.so
if [[ -f "bin/libphonon.so" ]]; then
    cp bin/libphonon.so "$INSTALL_DIR/"
    echo -e "${GREEN}Biblioteka libphonon.so zainstalowana.${NC}"
else
    echo -e "${YELLOW}Informacja: libphonon.so nie znaleziona w bin/, pomijam.${NC}"
fi

# 3. Pliki konfiguracyjne (.cfg)
if [ -d "config" ]; then
    echo -e "${YELLOW}Kopiowanie konfiguracji...${NC}"
    cp -v config/*.cfg "$INSTALL_DIR/baseq2/"
    echo -e "${GREEN}Pliki .cfg (w tym autoexec.cfg) zostały zainstalowane.${NC}"
fi

echo -e "${GREEN}Wszystkie pliki zostały pomyślnie rozmieszczone.${NC}"
echo
